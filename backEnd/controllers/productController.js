...
exports.getAllProducts = asyncErrors(async (req, res,next) => {
    ...
    //apllying additional filters on top of search
    const apiFeatures = new ApiFeatures(Product, req.query).search().filter().pagination(ResultsPerPage) //passing query and querystr
